...
function carregado(){  
    ...
    db.transaction(function(tx) {
      ...
      tx.executeSql("CREATE TABLE IF NOT EXISTS tabelapedido ( id INTEGER PRIMARY KEY,nome TEXT,pedido TEXT, quantidade TEXT, detalhes TEXT, situacao TEXT, horario TEXT, data TEXT, valor TEXT)");
...
function salvar(){
    ...
    db.transaction(function(tx) {
        if(id){
            tx.executeSql('UPDATE tabelapedido SET nome=?, pedido=?, quantidade=?, detalhes=?, situacao=?, data=?, valor=? WHERE id=?', [nome,ped,quant,det,sit,dat,totalval,id],null);
        }else{
            tx.executeSql('INSERT INTO tabelapedido ( nome,pedido,quantidade,detalhes,situacao,horario,data,valor) VALUES (?, ?, ?, ?, ?,?,?,?)', [nome,ped,quant,det,sit,hor,dat,totalval]);
        }
    });
